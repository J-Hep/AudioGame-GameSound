...
#pragma region Vertex n Fragments n textures
...
#pragma region Materials n Scenes n luts
		// Loading in a 1D LUT
...
#pragma region Lights Camera Plane Parents
		// Create some lights for our scene gg
		GameObject::Sptr lightParent = scene->CreateGameObject("Lights");

		GameObject::Sptr light = scene->CreateGameObject("Light");
		light->SetPostion(glm::vec3(0.f,0.f,10.f));
		lightParent->AddChild(light);

		Light::Sptr lightComponent = light->Add<Light>();
		lightComponent->SetColor(glm::vec3(1.0f));
		lightComponent->SetRadius(25);
		lightComponent->SetIntensity(50);
